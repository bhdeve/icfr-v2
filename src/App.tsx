...
import { billingService, surveysService } from "./api/services";
...
function AppContent(): JSX.Element {
  ...
  // Load surveys from backend when authenticated
  ...
  const loadSurveys = useCallback(async () => {
    ...
    try {
      ...
      const response = await surveysService.list({ page: 1, limit: 50 });
  ...
  const handleDeleteSurvey = (surveyId: string): void => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const response = await surveysService.delete(surveyId);
  ...
  const handleSurveyCreated = (survey: Survey): void => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const response = await surveysService.create(survey as any);
  ...
  const getAllSurveysFromDatabase = (): Survey[] => surveys;
