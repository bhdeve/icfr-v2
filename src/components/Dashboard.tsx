...
export const Dashboard: React.FC<DashboardProps> = ({ userRole }) => {
  ...
  const mainStats = useMemo(() => {
    ...
    return [
      {
        title: 'Total Surveys',
        value: overview ? String(overview.totalSurveys ?? 0) : '247',
        change: overview ? '+0%' : '+12.5%',
        trend: 'up',
        icon: FileText,
        color: '#18325A',
      ...
      },
      {
        ...
        value: overview ? String(overview.totalResponses ?? 0) : '3,842',
        change: overview ? '+0%' : '+23.1%',
        trend: 'up',
        icon: Users,
        color: '#10b981',
        bgColor: 'bg-green-50',
        description: 'Responses received across surveys'
      },
      {
        title: 'Completion Rate',
        value: overview ? `${overview.completionRate ?? 0}%` : '87.4%',
        change: overview ? '+0%' : '+5.2%',
        trend: 'up',
        icon: TrendingUp,
        color: '#8b5cf6',
        bgColor: 'bg-purple-50',
        description: 'Average completion across surveys'
      },
      {
        title: 'Total Beneficiaries',
        value: overview ? String(overview.totalBeneficiaries ?? 0) : '42',
        change: overview ? '+0' : '+8',
        trend: 'up',
        icon: Building2,
        color: '#f59e0b',
        bgColor: 'bg-amber-50',
        description: 'Beneficiaries engaged'
      }
    ];
  }, [analyticsData]);

  const responsesOverTime = useMemo(() => {
    const apiSeries = analyticsData?.charts?.responsesOverTime || [];
    if (apiSeries.length) {
      return apiSeries.map((item: any) => ({
        date: item.date,
        responses: item.count,
      }));
    }
    return [
      { date: 'Week 1', responses: 18 },
      { date: 'Week 2', responses: 22 },
      { date: 'Week 3', responses: 25 },
      { date: 'Week 4', responses: 28 },
      { date: 'Week 5', responses: 32 },
      { date: 'Week 6', responses: 29 }
    ];
  }, [analyticsData]);

  const responseRateData = useMemo(() => {
    const trend = analyticsData?.trends?.responses;
    if (trend) {
      return [
        { week: 'Previous', rate: trend.previous ?? 0 },
        { week: 'Current', rate: trend.current ?? 0 },
      ];
    }
    return [
      { week: 'Week 1', rate: 82 },
  ...
  const surveyStatusData = useMemo(() => {
    const overview = analyticsData?.overview;
    if (overview) {
      const active = overview.activeSurveys ?? 0;
      const total = overview.totalSurveys ?? 0;
    ...
    return [
      ...
      { name: 'Draft', value: 98, color: '#8b5cf6' },
      { name: 'Closed', value: 25, color: '#f59e0b' }
    ];
  }, [analyticsData]);

  const topOrganizations = useMemo(() => {
    const recent = analyticsData?.recentSurveys;
    if (recent?.length) {
    ...
    return [
      { name: 'O?U.O1USOc OU,O?U.U, OU,OrUSO?USOc', surveys: 45, responses: 823, rate: 92 },
      { name: 'U.OO3O3Oc OU,O?U+U.USOc OU,U.O?O?U.O1USOc', surveys: 38, responses: 712, rate: 89 },
      { name: 'O?U.O1USOc OU,O?O1OUSOc OU,OO?O?U.OO1USOc', surveys: 32, responses: 645, rate: 86 },
      { name: 'U.O?U?O? OU,O_O1U. OU,U.O?O?U.O1US', surveys: 28, responses: 534, rate: 84 },
      { name: 'O?U.O1USOc OU,O"O? U^OU,O?O-O3OU+', surveys: 24, responses: 467, rate: 81 }
    ];
  }, [analyticsData]);

  const beneficiariesGrowth = useMemo(() => {
    const trend = analyticsData?.trends?.beneficiaries;
    if (trend) {
      return [
        { month: 'Previous', total: trend.previous ?? 0, new: 0 },
        { month: 'Current', total: trend.current ?? 0, new: (trend.current ?? 0) - (trend.previous ?? 0) },
      ];
    }
    return [
      { month: 'Jan', total: 2845, new: 234 },
      { month: 'Feb', total: 3079, new: 289 },
      { month: 'Mar', total: 3368, new: 312 },
      { month: 'Apr', total: 3680, new: 345 },
      { month: 'May', total: 4025, new: 378 },
      { month: 'Jun', total: 3842, new: 298 }
    ];
  }, [analyticsData]);
  ...
  if (analyticsLoading && !analyticsData) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3 text-[#18325A]">
          <div className="w-10 h-10 border-4 border-[#18325A] border-t-transparent rounded-full animate-spin" />
          <p>Loading data...</p>
        </div>
      </div>
    );
