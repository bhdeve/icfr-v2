...
export function ForgotPasswordModal({ isOpen, onClose }: ForgotPasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) {
      toast.error('USO?O?U% O?O_OrOU, OU,O"O?USO_ OU,O?U,U?O?O?U^U+US');
      return;
    }

    if (!email.includes('@')) {
      toast.error('USO?O?U% O?O_OrOU, O"O?USO_ O?U,U?O?O?U^U+US O?O-USO-');
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        setStep('sent');
        toast.success('O?U. O?O?O3OU, O?OO"O? O?O1OO_Oc O?O1USUSU+ U?U,U.Oc OU,U.O?U^O?');
      } else {
        toast.error(response.error?.message || 'OU,O"O?USO_ OU,O?U,U?O?O?U^U+US O?USO? U.O3O?U, U?US OU,U+O,OU.');
      }
    } catch (error: any) {
      toast.error(error?.message || 'O-O_O? OrO?O?OO USO?O?U% OU,U.O-OU^U,Oc U.O?Oc O?OrO?U%');
    } finally {
      setIsLoading(false);
    }
  };
