...
import { beneficiariesService } from '../api/services';
...
export function OrganizationBeneficiariesPage({ onViewSurveys }: OrganizationBeneficiariesPageProps) {
  ...
  useEffect(() => {
    const loadBeneficiaries = async () => {
      try {
        const response = await beneficiariesService.list({ page: 1, limit: 50 });
        if (response.success && response.data) {
          const items = response.data.items || [];
          setBeneficiaries(items as any);
        } else {
  ...
  return (
    ...
    >
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        ...
        <DialogContent className="max-w-full sm:max-w-3xl mx-4">
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              عرض جميع معلومات وبيانات المستفيد بالتفصيل
