...
export function SurveyResultsPage({ surveyId, onBack, onBackToSurvey }: SurveyResultsPageProps) {
  ...
  const handleExport = (format: 'excel' | 'pdf') => {
    ...
    setTimeout(() => {
      ...
      toast.success(`تم تصدير النتائج بصيغة ${format === 'excel' ? 'Excel' : 'PDF'} بنجاح!`);
    }, 2000);
  };
    ...
    >
      <div className="space-y-4">
        ...
        <Dialog open={showExportDialog} onOpenChange={setShowExportDialog}>
          <DialogContent className="max-w-full sm:max-w-lg arabic-text mx-4" dir="rtl">
            <DialogHeader>
              <DialogTitle className="text-2xl text-[#183259]">تصدير نتائج الاستبيان</DialogTitle>
              <DialogDescription>
                اختر صيغة التصدير التي تناسبك لتحميل نتائج الاستبيان
              </DialogDescription>
            </DialogHeader>
