...
export function UserManagement({ userRole }: UserManagementProps) {
  ...
  const [userDrawerMode, setUserDrawerMode] = useState<'create' | 'edit' | 'view'>('create');
  ...
  // Dialog states for editing beneficiaries
  ...
  const handleExportUsers = () => {
    try {
      ...
      toast.error('حدث خطأ في تصد��ر البيانات');
    }
  };
  ...
  const handleSaveUser = (userData: User) => {
    ...
    } else if (userDrawerMode === 'edit' && selectedUser) {
      // Check for duplicate username or email with other users
      const usernameExists = users.some(user => 
        user.id !== selectedUser.id && user.username === userData.username
      );
      const emailExists = users.some(user => 
        user.id !== selectedUser.id && user.email === userData.email
      );
  ...
  const stats = [
    {
      title: userRole === 'admin' ? 'إجمالي المستفيدين' : 'إجمالي المستخدمين',
    ...
    {
      title: userRole === 'admin' ? 'المستفيدين النشطين' : 'المستخدمين النشطين',
      value: visibleUsers.filter(u => u.status === 'active').length.toString(),
      icon: UserCheck,
      iconColor: 'text-green-600',
      iconBgColor: 'bg-green-50'
    },
    {
      title: userRole === 'admin' ? 'إجمالي الاستبيانات' : 'في الانتظار',
      value: (userRole === 'admin' 
        ? visibleUsers.reduce((sum, user) => sum + (user as BeneficiaryData).surveyCount, 0)
        : visibleUsers.filter(u => u.status === 'pending').length
      ).toString(),
      icon: userRole === 'admin' ? FileText : Clock,
      iconColor: 'text-purple-600',
      iconBgColor: 'bg-purple-50'
    },
    {
      title: userRole === 'admin' ? 'إجمالي المكتملة' : 'غير النشطين',
      value: (userRole === 'admin' 
        ? visibleUsers.reduce((sum, user) => sum + (user as BeneficiaryData).completedResponses, 0)
        : visibleUsers.filter(u => u.status === 'inactive').length
      ).toString(),
      icon: userRole === 'admin' ? CheckCircle : UserX,
  ...
  return (
    <AdminPageLayout
      title={userRole === 'admin' ? 'إدارة المستفيدين' : 'إدارة المستخدمين'}
    ...
    >
      ...
      <Dialog open={isEditBeneficiaryOpen} onOpenChange={setIsEditBeneficiaryOpen}>
        <DialogContent className="max-w-full sm:max-w-md mx-4">
          <DialogHeader>
            <DialogTitle>تعديل بيانات المس��فيد</DialogTitle>
            <DialogDescription>
